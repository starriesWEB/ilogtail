...
func (f *FlusherClickHouse) BufferFlush(projectName string, logstoreName string, configName string, logGroupList []*protocol.LogGroup) error {
	...
	for _, logGroup := range logGroupList {
		...
		for _, log := range serializedLogs.([][]byte) {
			...
			err := json.Unmarshal(log, &unmarshaledLog)
			if err != nil {
				logger.Error(f.context.GetRuntimeContext(), "FLUSHER_FLUSH_ALARM", "flush clickhouse json.Unmarshal fail##, error##", err)
			}
			contentMap, ok := unmarshaledLog["contents"].(map[string]interface{})
			if !ok || contentMap == nil {
				logger.Warning(f.context.GetRuntimeContext(), "FLUSHER_FLUSH_ALARM", "contentMap is empty##", unmarshaledLog)
				return err
			}
			tagsMap, ok := unmarshaledLog["tags"].(map[string]interface{})
			if !ok || tagsMap == nil {
				logger.Warning(f.context.GetRuntimeContext(), "FLUSHER_FLUSH_ALARM", "tagsMap is empty##", unmarshaledLog)
				return err
			}

			content, _ := contentMap["content"].(string)
			containerName, _ := tagsMap["container.name"].(string)
			hostIp, _ := tagsMap["host.ip"].(string)
			hostName, _ := tagsMap["host.name"].(string)
			timestamp := uint64(unmarshaledLog["time"].(float64))

			sql := fmt.Sprintf("INSERT INTO %s.ilogtail_%s_buffer (_timestamp, hostName, hostIp, containerName, content) VALUES (%d, '%s', '%s', '%s', '%s')", f.Authentication.PlainText.Database, f.Table, timestamp, hostName, hostIp, containerName, content)
			logger.Debug(f.context.GetRuntimeContext(), "flush clickhouse sql###", sql)
...
func createNullBufferTable(f *FlusherClickHouse) error {
	...
	sqlNull := fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s (`_timestamp` Int64,`hostName` String, `hostIp` String, `containerName` String, `content` String) ENGINE = Null", sqlNullTableName)
